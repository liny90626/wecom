...
当前维护版本为 `3.0.0-6`，发布标签 `released/3.0.0-6`。**版本号说明**：规则仍是 `<上游基线>-<构建号>`；上游基线自本版起记为 `3.0.0`，构建号从 5 起。代码仍是本 fork 的 2.7.260 Bot WS 车道：上游 `v3.0.0` 是以腾讯官方插件为主线的整体重建，本 fork 只从中移植经过评估的部分（见 [`changelog/v3.0.0-5.md`](./changelog/v3.0.0-5.md)）。`2.7.260-3`、`3.0.0-v1`、`3.0.0-v2` 已撤回，`3.0.0-5` 含 MEDIA 文件丢失缺陷被 `3.0.0-6` 取代，历史提交保留。
