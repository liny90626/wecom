...
对 3.0.0-7～-10 的对抗式审核跟进：从未画过预览的流收尾改为「（回复完毕）」，不再把「⏳ 正在思考中...」留成气泡终态；修正一个只在 threads pool 下必红的 `~` 媒体路径用例；deferred 媒体用例补上「先传附件再关流」的顺序断言。经查企微 markdown-v2 子集没有任何删除线语法，3.0.0-8 保留单个 `~` 的决定成立。详见 [`changelog/v3.0.0-11.md`](./changelog/v3.0.0-11.md)。
