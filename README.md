...
当前已发布版本为 `v2.5.110-148`，发布标签 `released/2.5.110-148`。本轮把失败长任务的呈现补齐：模型服务返回无法归类的错误时，OpenClaw 只会给出一行 `LLM request failed.` 并丢弃原始细节，插件不再原样转发这一行，而是带上「本次任务未完成（已运行 X）」的框架与耗时，并新增诊断日志便于对齐网关日志里的真实原因；长任务进行中的提示也改为请用户耐心等待、尽量不要打断。上一版（v147）的内容：本轮针对现网反馈的稳定性问题：一次丢失的企业微信回执不再永久熄灭进度通道，思考块与过程信息在长任务中恢复正常；任务失败时错误文案不再抹掉用户正在看的推理过程；被新消息接管的旧回合不再把 OpenClaw 的失败文案当成新消息推给用户，会话接管也改为与 OpenClaw 接受中止在同一 tick 内发布，关闭了“已杀掉的 run 仍在线”的窗口。同时保留 v146 的入站即时占位、被接管消息并入、提示与事实对齐及 7.1 冻结中止有界等待。完整说明见 [`changelog/v2.5.110-147.md`](./changelog/v2.5.110-147.md)。
