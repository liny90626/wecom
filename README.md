...
当前已发布版本为 `v2.5.110-146`，发布标签 `released/2.5.110-146`。它以 `v2.5.110-118` 的低等待路径为参照做了一轮对抗式复核：收到消息即开占位气泡（文件上传不再等下载完成才有反馈），媒体合并窗口内也先确认并复用同一气泡；prepare 期间被新消息接管的旧消息，其正文与附件会并入后继消息而不再静默丢失；被 OpenClaw 并入运行中任务的消息不再被告知“未执行”，避免用户重发造成重复执行；OpenClaw 7.1 在投递阶段拒绝中止时，新指令会有界等待旧任务释放而不是直接拒收。同时保留 v146 第一轮的媒体+文字合并去重、pending/active 接管分层与 WS runtime owner 生命周期修复。完整说明见 [`changelog/v2.5.110-146.md`](./changelog/v2.5.110-146.md)。
