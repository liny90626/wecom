...
`v2.5.110-139` 面向 OpenClaw 2026.7.1 生产环境：正确识别“新消息被并入活跃任务 / 回合转后台续跑”的正常返回，不再误报“投递中断”，失败提示也不再泄露思考内容；补齐流窗过期后 final 投递的多条静默丢失路径（结果不明失败按分片进度有界补发、重试耗尽推送一次失败提示）；“任务仍在后台处理”提示改为任务处理满 9 分钟才出现。主线仍不恢复分钟级 handoff、重试阶梯、per-peer 熔断器或 synthetic thinking。当前发布标签为 `released/2.5.110-139`。
