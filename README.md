...
当前已发布版本为 `v2.7.260-2`，发布标签 `released/2.7.260-2`。**版本号说明**：规则仍是 `<上游基线>-<构建号>`；上游基线保持 `2.7.260`，构建号从 1 递增到 2。本轮根治两件来自同一条 Bot WS 失败链的问题。其一，上一轮刚完成时 reply operation 可能在核心返回与 active-run 查询之间释放；旧插件据此把正常的终态收尾误判为本次零输出，偶发提示「本次回复投递中断」。现在改用 `onAgentRunStart` / `onTurnAdopted` 记录本次调用是否真正被接收，active 查询只作诊断；未接收的 flagless zero 仍只走既有一次 500ms 重试，并且不再误 drain 可能来自 dedupe 的正常 run。其二，同一个错误先经 tracked `replyHandle.fail()` 记录一次，又逃到 WS frame 边界记录第二次；现在只收口「Bot WS + fail 已成功完成 + 同一错误对象」这一条已处理路径，其他异常继续上抛。完整说明见 [`changelog/v2.7.260-2.md`](./changelog/v2.7.260-2.md)。
