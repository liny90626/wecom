...
当前已发布版本为 `v2.7.260-3`，发布标签 `released/2.7.260-3`。**版本号说明**：规则仍是 `<上游基线>-<构建号>`；上游基线保持 `2.7.260`，构建号从 2 递增到 3。本轮恢复 OpenClaw 真实过程文字：插件此前没有订阅 `onItemEvent`，所以 commentary/preamble 在到达企业微信投递层之前就被丢掉，用户只能看到插件自己的长任务状态。现在通过 OpenClaw 6.11/7.1 共同支持的渠道进度契约接收并转发 preamble，按 item 合并累计快照，过滤工具内部状态，且继续使用既有非阻塞进度队列，慢回执不会反向阻塞模型。长任务辅助状态从每 15 秒降为每 60 秒，并缩短为 `【任务处理中，已用时 1m30s】`；首个占位确认和真实过程文字的即时让位保持不变。完整说明见 [`changelog/v2.7.260-3.md`](./changelog/v2.7.260-3.md)。
