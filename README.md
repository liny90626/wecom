...
当前已发布版本为 `v2.7.260-7`，发布标签 `released/2.7.260-7`。**版本号说明**：规则仍是 `<上游基线>-<构建号>`；上游基线保持 `2.7.260`，构建号递增到 7。`2.7.260-3` 的 tag 与包已经撤回，历史提交保留。

本轮补齐 `2.7.260-6` 未覆盖的重复形态：真实过程快照可能在一段内部重复某行、不同快照局部重叠，多个结构化事件脱敏后也可能显示成相同的 `Tool Call` / `Exec`。Bot WS 最终瞬态气泡现在统一按可见行精确去重，覆盖流式更新、后台状态和主动推送；不做模糊匹配、不转入思考块，也不改变成功 final 清理瞬态过程的既有语义。完整说明见 [`changelog/v2.7.260-7.md`](./changelog/v2.7.260-7.md)。
