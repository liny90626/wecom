...
当前已发布版本为 `v2.7.260-9`，发布标签 `released/2.7.260-9`。**版本号说明**：规则仍是 `<上游基线>-<构建号>`；上游基线保持 `2.7.260`，构建号递增到 9。`2.7.260-3` 的 tag 与包已经撤回，历史提交保留。

本轮把长任务状态 `【长任务处理中，请勿打断，已用时X】` 的刷新间隔改为 **60 秒**，并根治它此前忽长忽短、偶尔重复的问题。根因不是间隔值：这条状态有三个可能的绘制者（气泡心跳、冻结刷新、后台推送），却各自持有定时器，相位由最近一次无关事件（一帧过程文字、一次外部推送、一次回执缺失）决定。现在统一到一条以回合开始时间为锚的**绝对网格**，三条通道绘制前查询、绘制后回报；名额在发出时占用，只有企微明确拒绝该帧时才归还。完整说明见 [`changelog/v2.7.260-9.md`](./changelog/v2.7.260-9.md)。
