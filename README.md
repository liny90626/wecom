...
`v2.5.110-144` 修复 OpenClaw 已确认外部答案投递后，旧 Bot WS ReplyHandle 仍普通收口并误启动 final 补发的问题：外部投递成功现在直接复用既有静默结算路径，不再让过期原流触发 `20/40/80s` 重试及错误的“本次回复投递中断”提示；旧流的 block/tool/final 错误也不会覆盖已经提交的外部答案。生产实现没有新增状态字段、timer、轮询或重试。沿用 v143 的暖会话 metadata 解阻塞、错误 final 正确收口及依赖安全补丁。当前发布标签为 `released/2.5.110-144`。
...
#### 📌 v2.5.110-144（2026-07-23，LinKy fork 维护版）
...
#### 📌 v2.5.110-139（2026-07-17，LinKy fork 维护版）
- **[7.1 适配] 并入/排队/yield 不再误报“投递中断”**：OpenClaw 2026.7.1 会把繁忙会话的新消息并入活跃 run 或排队、回合也可能转后台续跑，dispatch 以“无可见输出”正常返回；现按“有活动→静默收口、被并入→即时提示用户、真死寂→保底失败”三路分诊，失败提示不再把 `<think>` 思考内容剥壳成可见英文碎片。
- **[投递抗丢失] 流窗过期后 final 不再静默消失**：结果不明的推送失败（ACK/本地超时、断连）改为保留投递认领并按分片进度有界补发（最多 3 次，仅补未确认分片）；重试耗尽推送一次“投递中断”提示；新消息不再销毁“一个分片都没送达”的 final 重试；纯思考气泡被接管时旧任务真实答案改走主动推送。
- **[接管硬化] 修复孤儿 drain 竞态、不引入 forceClear**：被接管 dispatch 的冲突重试先查 abort，杜绝误杀后继消息刚启动的 run；预派发排空仅优雅 abort（7.1 投递期冻结 abort 属正常，强清会把健康任务打成失败）。
- **[体验] 后台处理提示延至 9 分钟**：流窗约 6 分钟死亡后，“任务仍在后台处理，完成后将以新消息发送”挂起到任务处理满 9 分钟再发；final 先到或被接管则彻底跳过，推送失败自动补试一次。完整说明见 [`changelog/v2.5.110-139.md`](./changelog/v2.5.110-139.md)。
