...
当前已发布版本为 `v2.7.260-1`，发布标签 `released/2.7.260-1`。**版本号说明**：规则仍是 `<上游基线>-<构建号>`；本轮合并了上游 `YanHaidao/wecom` 的 `2.7.260`，基线随之变更，构建号按规则从 1 重新计数（上一版是 `2.5.110-149`）。本轮做了三件事。其一，长任务反馈改为**时间驱动**：OpenClaw 默认不流式推理，一个纯工具执行的回合不会产生任何流式回调，此前插件的三条反馈通道全是内容驱动，于是同时静默——现网表现就是「十分钟只有一句『正在思考...』，最后回一行英文错误」；现在满 30 秒即切换为带耗时的专注提示并降到低频节奏，且这是**净减法**（删掉了 120 秒保活上限与两个定时器，10 分钟共 33 帧，而旧实现光前 2 分钟就发 40 帧）。其二，堵住一处会**静默销毁完整答案**的取消逻辑：回合答完但推送失败（用户一个字都没收到）时，只要用户在重试窗口内发了下一条消息，那份完整答案就会被永久销毁——它不需要接管也不需要中止，现在只有「已有分片确认送达」才会取消重试。其三，合并上游新提交并逐条处置分歧点（`peerDependencies` 不跟随上游收窄、运行时依赖保持精确钉版），顺带修正了上游一个在 registry 上并不存在、会让干净安装 404 的 `@types/node` 版本声明。完整说明见 [`changelog/v2.7.260-1.md`](./changelog/v2.7.260-1.md)。
...
#### 📌 v2.5.110-148（2026-07-30，LinKy fork 维护版）

- **[失败的长任务不再只剩一行英文]**：长任务的工作常常全是工具调用（没有可见正文），失败时企微流窗口也早已关闭，final 只能走主动推送——那条路线过去原样转发 `LLM request failed.`。现在会带上「⚠️ 本次任务未完成（已运行 12m30s）」的框架与耗时；推理内容仍只在折叠块内展示，不提升为可见正文。
- **[失败可诊断]**：新增 `[wecom-reply] error-final` 日志，记录耗时、正文/思考字数与投递路线。`LLM request failed.` 是 OpenClaw 对无法归类的模型错误的固定文案，原始细节被它有意丢弃，用这条日志的时间戳与 reqId 去对齐网关日志即可拿到真实原因。
- **[长任务提示词]**：改为「正在专注任务中，请耐心等待尽量不要打断我，若10分钟我未发出任何消息再发消息来咨询，万分感谢。 当前长任务用时X」，冻结气泡的状态后缀与 9 分钟后的后台推送同时生效。
- **[验证]**：现网症状先精确复现再修复；另单独复现并修掉一处重试身份漂移（失败文案里的耗时会让重试重推已送达分片）。全量测试 492/492，类型检查、构建、产物验证及 B1/B2/B3 全部通过，均在生产同版本 OpenClaw `2026.7.1` 上完成。完整说明见 [`changelog/v2.5.110-148.md`](./changelog/v2.5.110-148.md)。
