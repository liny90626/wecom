...
当前已发布版本为 `v2.5.110-149`，发布标签 `released/2.5.110-149`。本轮修复三件事：文件与文字无论先后都能合并成同一个回合（此前先打字会丢附件、先发附件会丢指令）；长任务运行中的产出随「正在专注任务中…」提示一起送达，不再被丢弃到最终答复；以及一次主动推送（OpenClaw 子任务完成通知是最常见来源）会永久熄灭该长任务全部进度反馈的缺陷。上一版（v148）的内容：本轮把失败长任务的呈现补齐：模型服务返回无法归类的错误时，OpenClaw 只会给出一行 `LLM request failed.` 并丢弃原始细节，插件不再原样转发这一行，而是带上「本次任务未完成（已运行 X）」的框架与耗时，并新增诊断日志便于对齐网关日志里的真实原因；长任务进行中的提示也改为请用户耐心等待、尽量不要打断。上一版（v147）的内容：本轮针对现网反馈的稳定性问题：一次丢失的企业微信回执不再永久熄灭进度通道，思考块与过程信息在长任务中恢复正常；任务失败时错误文案不再抹掉用户正在看的推理过程；被新消息接管的旧回合不再把 OpenClaw 的失败文案当成新消息推给用户，会话接管也改为与 OpenClaw 接受中止在同一 tick 内发布，关闭了“已杀掉的 run 仍在线”的窗口。同时保留 v146 的入站即时占位、被接管消息并入、提示与事实对齐及 7.1 冻结中止有界等待。完整说明见 [`changelog/v2.5.110-147.md`](./changelog/v2.5.110-147.md)。
