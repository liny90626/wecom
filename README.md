...
当前已发布版本为 `v2.7.260-12`，发布标签 `released/2.7.260-12`。**版本号说明**：规则仍是 `<上游基线>-<构建号>`；上游基线保持 `2.7.260`，构建号递增到 12。`2.7.260-3` 的 tag 与包已经撤回，历史提交保留。

本轮起因是一次**上游核查**：企微在 2026-08 重写了整套智能机器人协议文档，并首次发布了官方 OpenClaw 插件与 wecom-cli。核查结论是 `@wecom/aibot-node-sdk` **无新版本可升**（1.0.7 仍是 latest，本仓库已锁定），但对照新文档与 SDK 实际源码行为后发现三件事并已修复：**被企微踢下线时，插件把断连通知当成一条用户消息跑了一轮 agent**（企微规定同一机器人只能有一条长连接，新连接会挤掉旧连接并先推 `disconnected_event`；该帧没有发送者也没有会话，派发出去的 peer 落成 `unknown`，而「被踢」的诊断因为关键词早已过时从不触发）；**`undici` 的 1 个 high 告警**（7.28.0 → 7.29.0，产线代码零改动）；**主动推送没带 `chat_type`**，等于每条都让服务端先按群聊猜一次，而入站帧早就告诉了我们答案。完整说明见 [`changelog/v2.7.260-12.md`](./changelog/v2.7.260-12.md)。
