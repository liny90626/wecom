...
当前已发布版本为 `v2.7.260-10`，发布标签 `released/2.7.260-10`。**版本号说明**：规则仍是 `<上游基线>-<构建号>`；上游基线保持 `2.7.260`，构建号递增到 10。`2.7.260-3` 的 tag 与包已经撤回，历史提交保留。

本轮根治「长任务过程在同一个气泡上覆盖刷写、既读不到也留不下」：过程文字从**可变状态**（只存当前步骤、帧帧覆盖、final 不含）改为**追加式步骤日志**——OpenClaw 源头本就把自述作为一次性有序段落发射，一条 commentary item 就是一步。气泡内按 `1）2）…` 编号追加显示；推送通道按 durable 书签送增量；final 送达后，未持久化的步骤以一条「📋 本轮过程记录」推送落档（回合 ≥2 分钟且 ≥2 步才推）。完整说明见 [`changelog/v2.7.260-10.md`](./changelog/v2.7.260-10.md)。
