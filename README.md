...
当前已发布版本为 `v2.5.110-147`，发布标签 `released/2.5.110-147`。本轮针对现网反馈的稳定性问题：一次丢失的企业微信回执不再永久熄灭进度通道，思考块与过程信息在长任务中恢复正常；任务失败时错误文案不再抹掉用户正在看的推理过程；被新消息接管的旧回合不再把 OpenClaw 的失败文案当成新消息推给用户，会话接管也改为与 OpenClaw 接受中止在同一 tick 内发布，关闭了“已杀掉的 run 仍在线”的窗口。同时保留 v146 的入站即时占位、被接管消息并入、提示与事实对齐及 7.1 冻结中止有界等待。完整说明见 [`changelog/v2.5.110-147.md`](./changelog/v2.5.110-147.md)。

开发与测试依赖已升级至生产同版本 OpenClaw `2026.7.1`，`peerDependencies` 仍保留 `^2026.6.11`，代码继续双版本兼容。
...
#### 📌 v2.5.110-147（2026-07-29，LinKy fork 维护版）

- **[思考块与过程信息恢复]**：企业微信同一 `req_id` 只允许一帧等待回执，任意一帧 5 秒未回执原本会永久熄灭整轮进度通道，用户全程只看到“正在思考中”。现在只有流真正失效（846605/846608）才退休通道，回执缺失仅让需要证明送达的动作改走主动推送。
- **[失败也保留过程]**：错误 final 会带上用户正在看的推理块，不再把气泡覆盖成一行英文错误；推理块只占用该帧剩余的字节预算，不改变答案自身的分段。
- **[不再有莫名的失败提示]**：被新消息接管的回合，其核心失败文案不再以新消息推送（真实答案与带媒体的回复仍照常送达）。
- **[接管窗口收紧]**：会话接管改为在 OpenClaw 接受中止的同一 tick 内发布，消除“旧 run 已被杀掉但仍能投递”的空窗；中止被拒绝（健康任务正在收尾）时完全不动旧回合，交接轮询间隔从 150ms 收紧到 50ms。
- **[验证]**：新增忠实建模企业微信 SDK 投递层的回归套件，三条现网反馈均先复现再修复；全量测试 490/490，类型检查、构建、产物验证及 B1/B2/B3 全部通过，均在生产同版本 OpenClaw `2026.7.1` 上完成。完整说明见 [`changelog/v2.5.110-147.md`](./changelog/v2.5.110-147.md)。

#### 📌 v2.5.110-146（2026-07-25，LinKy fork 维护版）

- **[首反馈回到 v118 水平]**：占位气泡从“接管完成后”提前到“入站被接受时”，文件上传不再整个下载期无反馈；媒体合并窗口内也先确认，并让合并后的回合复用同一气泡。
- **[消息不再静默丢失]**：prepare 期间被接管的旧消息，其正文与附件并入后继消息（严格同发送者、普通消息类型）；不可合并时明确提示“本条尚未开始处理，请重新发送”，不再谎称已合并。
- **[提示与事实一致]**：被 OpenClaw steer/入队的消息回复“已并入当前任务”，恢复 `dispatch-absorbed-by-active-run` 日志；“确认新指令未执行”只用于核心确实拒绝的路径，避免用户重发造成重复执行。
- **[7.1 冻结中止不再拒收]**：终态提交后 `freezeAbort` 使中止被拒，新指令改为最长 3 秒有界等待旧任务释放（仍在既有 5 秒屏障内），超时才拒收；空 final 也不再把气泡清空。
- **[入站合并与属主生命周期]**：保留媒体+文字合并去重与群聊发送者隔离、pending/active 接管分层、ReplyHandle 绑定 WS runtime owner 及仅限明确未接收结果的单次 busy 重试。
- **[验证]**：全量测试 476/476，类型检查、构建、产物验证及 B1/B2/B3 全部通过，且全部在生产同版本 OpenClaw `2026.7.1` 上完成。完整说明见 [`changelog/v2.5.110-146.md`](./changelog/v2.5.110-146.md)。

#### 📌 v2.5.110-145（2026-07-24，LinKy fork 维护版）

- **[终态闭环]**：外部 final 已可见时同步取消旧 ReplyHandle 中已排队或在途的 final retry，旧链不会再次补发半截正文或重新挂定时器。
- **[作用域隔离]**：placeholder keepalive 改为按账号、会话类型和 peer 分组，同 userid 的多账号会话及 direct/group 不再互相停止保活。
- **[属主安全]**：WS push handle 与 account runtime 注销必须匹配原注册对象，热重载中的旧实例无法删除已经接管的新实例。
- **[来源稳定性]**：已知账号的 Bot WS/Agent 来源只查本账号；有界 source registry 正确刷新活跃快照的淘汰顺序。完整说明见 [`changelog/v2.5.110-145.md`](./changelog/v2.5.110-145.md)。

#### 📌 v2.5.110-144（2026-07-23，历史版本，发布标签已撤销）

- **[外部 final 结算]**：OpenClaw 6.11/7.1 确认 source reply 已通过其他路径可见后，原 Bot WS 流只做本地静默结算，不再重新投递进度或正文。
- **[消除误报重试]**：过期原流不再为已成功外发的答案启动 `20/40/80s` final retry，也不会在重试耗尽后错误推送“本次回复投递中断”。
- **[错误优先级收敛]**：外部答案已提交时，旧流遗留的 block/tool/final 错误不再覆盖成功结果；真实未投递 final 的既有补发和失败提示保持不变。
- **[最小生产改动]**：生产逻辑仅调整 `reply-orchestrator` 收口顺序，复用已有 `wecomExternalFinalDelivered` 语义，没有新增状态机、timer、轮询或网络等待。完整说明见 [`changelog/v2.5.110-144.md`](./changelog/v2.5.110-144.md)。
