...
修 **`MEDIA:` 指令发出的文件静默丢失**，即「agent 说文件发了，实际没到，再要一次才到」。根因：`2.7.260-26` 为兼容 2026.8.x 把本地媒体读取改走 fs-safe 的 `readLocalFileFromRoots`，Windows 现网上它把合法路径（含 OpenClaw 自己的 outbound 媒体目录）判为「不在允许目录内」，失败只拼进 final、没有日志；`message` 工具那条链路多合并了核心传入的根目录才偶尔成功。现在本地重写 -25 的 realpath 白名单语义，不依赖私有化的 SDK 函数；`MEDIA:` 链路补上 `[wecom-media] sent / send-failed` 日志。影响 9/2 12:51 至 `3.0.0-5` 的所有版本。详见 `changelog/v3.0.0-6.md`。
