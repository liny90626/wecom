...
当前已发布版本为 `v2.7.260-6`，发布标签 `released/2.7.260-6`。**版本号说明**：规则仍是 `<上游基线>-<构建号>`；上游基线保持 `2.7.260`，构建号递增到 6。`2.7.260-3` 的 tag 与包已经撤回，历史提交保留。

本轮修复 OpenClaw 不同 commentary item 携带完全相同 preamble 时，企微过程气泡把同一句话拼接两次的问题。展示层现在按完整文本精确去重，保留首个 item 的顺序以及各 item 后续更新能力；不做模糊匹配、不转入思考块，也不改变成功 final 清理瞬态过程的既有语义。完整说明见 [`changelog/v2.7.260-6.md`](./changelog/v2.7.260-6.md)。
