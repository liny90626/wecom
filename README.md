...
当前已发布版本为 `v2.7.260-11`，发布标签 `released/2.7.260-11`。**版本号说明**：规则仍是 `<上游基线>-<构建号>`；上游基线保持 `2.7.260`，构建号递增到 11。`2.7.260-3` 的 tag 与包已经撤回，历史提交保留。

本轮收口两条现网反馈。**思考块过长导致回复失败**：它是唯一没做 wire 安全化的文本——裸 `<` 会让客户端把 `</think>` 当成标签吃掉，块不闭合并连答案一起吞掉；同时它按**头部**截断，推理越过上限后每帧逐字节相同、被等值判定静音，气泡就停在某个长度不动。现改为走正文同源归一化 + 残留 `<` 中性化，并显示**最新的一段**；正文在场时思考块收紧到 800 字符（两者共用同一帧预算）。**长任务过程仍在丢失、收尾记录令人困惑**：流窗一死推送立刻接手（消除 6→8 分钟盲区），被企微拒绝的气泡帧视为永久记录、其步骤并入 durable 书签不再重复推送，取消「📋 本轮过程记录」。另外把消息上限按官方 20480 字节调到约 75%（6000 字答案由 4 条降到 2 条），并新增沉默看门狗让工具阶段的气泡不再假死。完整说明见 [`changelog/v2.7.260-11.md`](./changelog/v2.7.260-11.md)。
