...
#### 📌 v2.5.110-109（2026-06-27，LinKy fork 维护版）
- **[Reasoning 调试] 轮转 progress think 探针** 🔎 按企业微信 Bot WS progress 链路验证 `<think>...</think>` 的触发边界，用于定位思考块只在中间态 stream 中生效的问题。
- **[实验收敛] 保持 final 正文不依赖思考块语法** 🧩 继续让最终正文走普通正文路径，避免 debug 探针或思考标签污染正式回复。
