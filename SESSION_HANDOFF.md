...
> 最后更新：2026-08-11（`2.7.260-10` 发布收口）。新会话开工前先读本文件、`README.md`、`changelog/README.md` 与最新版本简报。

## 1. 当前状态

- 当前正式版本：**`2.7.260-10`**，发布标签 `released/2.7.260-10`，包 `yanhaidao-wecom-2.7.260-10.tgz`（228,894 bytes；解包 1,046,613 bytes；139 文件；npm shasum `0782f45f28a70be108bd626fbe05eff9f3fc6b27`；SHA-256 `41c6b851f5642a0c19a6433c2cdd8e44493a5d1d1d66641606224afac883d28e`）。根治「长任务过程在同一个气泡上覆盖刷写、既读不到也留不下」：过程文字从可变状态改为**追加式步骤日志**——气泡按编号追加显示，推送通道按 durable 书签送增量，final 送达后未持久化步骤以一条「📋 本轮过程记录」推送落档（≥2 分钟且 ≥2 步）。详见第 2o 节。
- 上一正式版本：`2.7.260-9`，包 `yanhaidao-wecom-2.7.260-9.tgz`（npm shasum `f104d4e133d300a9307ef1b24064b67225b1b674`）。长任务状态刷新间隔 60 秒、唯一网格时钟，详见第 2n 节。
...
41. **过程日志的持久化只有一个 durable 前缀书签**（v10）：所有推送通道（冻结/死流状态推送、owner 丢失强推、final 后落档）一律从 `processLogDurable` 起送增量，**确认送达才推进**（失败推送不推进——提前推进＝步骤从记录里静默消失）；不得为「气泡已展示」引入第二个书签或区间集合（会造成不连续区间与漏记，这正是本轮开发中试过并否决的方案）。落档必须在 final **送达之后**（不得抢答案的先手），superseded 不落档，记录推送不得复用 final 的重试身份机制。
...
- **真机（企微网关/客户端）验证仍未做**：`2.7.260-10` 发布后需确认——长任务过程在气泡内**按编号追加**而不再覆盖刷写；final 后收到**一条**「📋 本轮过程记录」推送（≥2 分钟且 ≥2 步的回合），步骤完整、编号连续、无重复；短问答（<2 分钟）**没有**多余的记录推送；长任务状态仍稳定每 60 秒一次且无双通道重复；过程气泡里仍无任何工具调用标签。另需继续观察 8 分钟心跳、相邻气泡、复用 `req_id`、迟到 ACK 与 emoji 边界。
