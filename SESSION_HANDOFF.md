...
> 最后更新：2026-08-26（`2.7.260-16` 构建完成，待真机验证后发布）。新会话开工前先读本文件、`README.md`、`changelog/README.md` 与最新版本简报。

## 1. 当前状态

- **`2.7.260-16` 已构建但未打 tag、未推远端**——`851003` 的根因已用真实端点坐实（两台不同的 MCP Server），详见第 2u 节。
- `2.7.260-15` 已被 `-16` 取代（同为未发布的验证版）——`-14` 冒烟失败后的收口，等真机验证通过再发布，详见第 2t 节。
