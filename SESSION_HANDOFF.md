...
- **未发布**：分支 `fix/v150-time-driven-long-task-feedback`，含三部分——根治「零产出长任务全程静默」（第 2d 节）、「漏消息」审计与修复（第 2e 节）、**合并上游 `YanHaidao/wecom` 并按表处置分歧点**（第 2f 节）。版本号已按既有规则推进到 **`2.7.260-150`**（基线跟上上游的 2.7.260，构建号继续递增）。**尚未打包 / 打 tag / 改 README 与 changelog / 推远端**，等待批准。
...
| 字段 | 上游 | 合并前的我们 | 处置 | 原因 |
| --- | --- | --- | --- | --- |
| `version` | `2.7.260` | `2.5.110-149` | **`2.7.260-150`** | 规则不变（`上游基线-我们的构建号`）：基线跟上游走，构建号继续单调递增 |
...
**版本号规则（不变）**：`<上游基线>-<我们的构建号>`。构建号自 v118 起单调递增，跨基线继续递增，绝不重置——v149 是 `2.5.110-149`，合并上游 2.7.260 后 v150 是 `2.7.260-150`。tag 为 `released/<完整版本号>`，简报为 `changelog/v<完整版本号>.md`。
