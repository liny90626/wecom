...
- 已发布正式版本：**`2.7.260-1`**，发布标签 `released/2.7.260-1`，包 `PACK_PLACEHOLDER`。开发分支 `fix/v150-time-driven-long-task-feedback` 已合入 `main` 并推送 `fork`。本版含三部分——根治「零产出长任务全程静默」（第 2d 节）、「漏消息」审计与修复（第 2e 节）、**合并上游 `YanHaidao/wecom` 并按表处置分歧点**（第 2f 节）。
