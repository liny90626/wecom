...
> 最后更新：2026-08-26（`2.7.260-12` 发布收口）。新会话开工前先读本文件、`README.md`、`changelog/README.md` 与最新版本简报。

## 1. 当前状态

- 当前正式版本：**`2.7.260-12`**，发布标签 `released/2.7.260-12`，包 `yanhaidao-wecom-2.7.260-12.tgz`（打包指纹见 `changelog/v2.7.260-12.md`）。一轮**上游核查**的产出：SDK 无新版本可升，但对照 2026-08 重写的协议文档与 SDK 1.0.7 实际源码后修掉三件事——被踢下线时断连事件被当成用户消息跑了一轮 agent 且 `ws-kicked` 从不落账、`undici` 的 1 个 high、主动推送未带 `chat_type`。详见第 2q 节。
- 上一正式版本：`2.7.260-11`，包 `yanhaidao-wecom-2.7.260-11.tgz`（234,268 bytes；npm shasum `e14aa513c4717bfbe2120ac131a0a46e4309ec76`；SHA-256 `b84d65db6d007ef1bb512e11aa616f7148aca74e5eb1e97b42b4fb1546a5dee7`）。思考块 wire 安全化与长任务过程收口，详见第 2p 节。
