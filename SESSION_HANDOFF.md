...
- 当前正式版本：**`2.7.260-9`**，发布标签 `released/2.7.260-9`，包 `yanhaidao-wecom-2.7.260-9.tgz`（225,000 bytes；解包 1,033,630 bytes；139 文件；npm shasum `f104d4e133d300a9307ef1b24064b67225b1b674`；SHA-256 `435c9b724d9b315bb33fa759ebe766ceca2ec2876f1325a2c19fdf93dd9185c2`）。长任务状态刷新间隔改为 **60 秒**，并把它统一到一条以回合开始时间为锚的绝对网格上（此前三条通道各持一个定时器，相位被无关事件重置，实测会出现 5s 重复推送与 15/22s 摆动）。详见第 2n 节。
