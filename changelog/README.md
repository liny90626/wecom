...
- [`v2.7.260-3`](./v2.7.260-3.md)（发布标签 `released/2.7.260-3`）：恢复 OpenClaw commentary/preamble 真实过程文字，根因是插件此前未订阅 `onItemEvent`；过程快照按 item 合并、保持顺序并沿既有非阻塞进度队列投递。长任务辅助状态从每 15 秒降为每 60 秒，文案缩短为 `【任务处理中，已用时 X】`，真实过程到达后仍立即让位。
