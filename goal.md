...
> 状态：**`2.7.260-20` 实现、自测、打包与 fork 推送已完成**。`2.7.260-19` 已完成 CLI 接入、窄范围 MCP 兜底、官方 Skills、打包与 fork 推送，但发布后对抗检查又发现多账号 `default` 别名和 CLI 错误输出脱敏边界；`-20` 以最小改动修正。Windows 真机/企业网关验证仍是后续环境门禁。
