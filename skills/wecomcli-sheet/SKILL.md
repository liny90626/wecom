...
`wecom-cli` 是插件提供的 tool，不是终端命令：所有操作都通过调用该 tool 完成，参数应使用 `args` 传入，禁止通过 exec / shell / 终端等命令行方式使用。

⚠️ 前置条件：首次调用 wecom-cli 前，必须按 wecomcli-preflight 技能执行前置条件检查，确保工具已加入白名单。
