...
import type { ResolvedUpstreamCorp } from "./upstream.js";
...
/** Exchange the primary enterprise token for a downstream enterprise token. */
export async function getUpstreamAccessToken(params: {
    primaryAgent: ResolvedAgentAccount;
    upstream: ResolvedUpstreamCorp;
}): Promise<string> {
    const { primaryAgent, upstream } = params;
    const cacheKey = JSON.stringify([
        "upstream",
        primaryAgent.accountId,
        primaryAgent.corpId,
        upstream.corpId,
        upstream.agentId,
    ]);
...
function primaryAuth(agent: ResolvedAgentAccount): AgentApiAuth {
    return { agentId: agent.agentId, getToken: () => getAccessToken(agent) };
}

function requireAuthAgentId(auth: AgentApiAuth, agent: ResolvedAgentAccount): number {
    return auth.agentId ?? requireAgentId(agent);
}
...
function upstreamAuth(
    ...
    return {
        agentId: upstream.agentId,
        getToken: () => getUpstreamAccessToken({ primaryAgent, upstream }),
    };
}
 ...
 */
async function sendTextWithAuth(params: {
    ...
    chatId?: string;
    text: string;
}, auth: AgentApiAuth): Promise<void> {
    ...
    const body = useChat
        ...
        : {
            ...
            totag: toTag,
            msgtype: "text",
            agentid: requireAuthAgentId(auth, agent),
            text: { content: text }
        };
...
export async function sendText(params: {
    ...
    return sendTextWithAuth(params, primaryAuth(params.agent));
}

export async function sendUpstreamText(params: {
    primaryAgent: ResolvedAgentAccount;
    upstream: ResolvedUpstreamCorp;
    toUser: string;
    text: string;
}): Promise<void> {
    return sendTextWithAuth(
        { agent: params.primaryAgent, toUser: params.toUser, text: params.text },
        upstreamAuth(params.primaryAgent, params.upstream),
    );
}
 ...
 */
async function uploadMediaWithAuth(params: {
    agent: ResolvedAgentAccount;
    type: "image" | "voice" | "video" | "file";
    buffer: Buffer;
    filename: string;
}, auth: AgentApiAuth): Promise<string> {
    ...
    const uploadId = diagnosticFingerprint(`${safeFilename}:${buffer.length}:${type}`);
    const startedAt = Date.now();

    console.log(
        `[wecom][agent-upload] stage=start upload=${uploadId} type=${type} filename=${diagnosticFingerprint(safeFilename)} bytes=${buffer.length}`,
    );
    ...
    const uploadOnce = async (fileContentType: string) => {
        ...
        console.log(
            `[wecom][agent-upload] stage=request upload=${uploadId} contentType=${fileContentType} bodyBytes=${body.length}`,
        );
        ...
        console.log(
            `[wecom][agent-upload] stage=response upload=${uploadId} status=${res.status} errcode=${String(json.errcode ?? 0)} hasMediaId=${Boolean(json.media_id)} durationMs=${Date.now() - startedAt}`,
        );
        return json;
    };
    ...
    if (!json?.media_id && preferredContentType !== "application/octet-stream") {
        console.warn(
            `[wecom][agent-upload] stage=retry upload=${uploadId} fromContentType=${preferredContentType} toContentType=application/octet-stream errcode=${String(json?.errcode ?? "unknown")}`,
        );
    ...
    console.log(
        `[wecom][agent-upload] stage=complete upload=${uploadId} type=${type} durationMs=${Date.now() - startedAt}`,
    );
...
export async function uploadMedia(params: {
    ...
    return uploadMediaWithAuth(params, primaryAuth(params.agent));
}

export async function uploadUpstreamMedia(params: {
    primaryAgent: ResolvedAgentAccount;
    upstream: ResolvedUpstreamCorp;
    type: "image" | "voice" | "video" | "file";
    buffer: Buffer;
 ...
 */
async function sendMediaWithAuth(params: {
    agent: ResolvedAgentAccount;
    toUser?: string;
    toParty?: string;
    toTag?: string;
...
export async function sendMedia(params: {
    ...
    return sendMediaWithAuth(params, primaryAuth(params.agent));
}

export async function sendUpstreamMedia(params: {
    primaryAgent: ResolvedAgentAccount;
    upstream: ResolvedUpstreamCorp;
    toUser: string;
    mediaId: string;
 ...
 */
async function downloadMediaWithAuth(params: {
    agent: ResolvedAgentAccount;
    mediaId: string;
    maxBytes?: number;
}, auth: AgentApiAuth): Promise<{ buffer: Buffer; contentType: string; filename?: string }> {
...
export async function downloadMedia(params: {
    ...
    return downloadMediaWithAuth(params, primaryAuth(params.agent));
}

export async function downloadUpstreamMedia(params: {
    primaryAgent: ResolvedAgentAccount;
    upstream: ResolvedUpstreamCorp;
    mediaId: string;
    maxBytes?: number;
}): Promise<{ buffer: Buffer; contentType: string; filename?: string }> {
    return downloadMediaWithAuth(
        {
            agent: params.primaryAgent,
            mediaId: params.mediaId,
