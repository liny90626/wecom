...
async function processAgentMessage(params: {
    ...
    const sendReplyText = async (text: string): Promise<void> => {
        const chunks = chunkTextToByteLimit(text, LIMITS.TEXT_MAX_BYTES, (value, charLimit) => {
            const parts: string[] = [];
        ...
        for (const chunk of chunks) {
            await paceSend();
            if (upstream) {
                await sendUpstreamText({ primaryAgent: agent, upstream, toUser: fromUser, text: chunk });
            } else {
                await sendText({ agent, toUser: fromUser, chatId: undefined, text: chunk });
            }
        }
    };
            ...
            deliver: async (payload: { text?: string; mediaUrls?: string[]; mediaUrl?: string }, info: { kind: string }) => {
                ...
                for (const mediaPath of mediaUrls) {
                    try {
                        ...
                        // 降级：发文本通知用户
                        try {
                            const filename = path.basename(mediaPath) || path.win32.basename(mediaPath) || "附件";
                            await sendReplyText(`⚠️ 文件发送失败：${filename}，请稍后重试。`);
                        } catch (notifyErr) {
                            error?.(`[wecom-agent] media failure notification failed: ${String(notifyErr)}`);
                        }
                    }
                }
