...
export function processDynamicRouting(params: DynamicRoutingParams): DynamicRoutingResult {
  ...
  if (useDynamicAgent) {
    log?.(
      `[dynamic-routing] 原始路由信息: agentId=${route.agentId}, matchedBy=${route.matchedBy}, session=${diagnosticFingerprint(route.sessionKey)}`,
    );
    ...
    log?.(
      `[dynamic-routing] 🔄 路由注入: agentId=${targetAgentId}, session=${diagnosticFingerprint(targetSessionKey)}`,
    );
