...
export function checkGroupPolicy(params: {
  ...
  if (!groupAllowed) {
    runtime.log?.(
      `[wecom][policy] account=${account.accountId} outcome=blocked policy=${groupPolicy} chat=${diagnosticFingerprint(chatId)} sender=${diagnosticFingerprint(senderId)}`,
    );
  ...
  if (!senderAllowed) {
    runtime.log?.(
      `[wecom][policy] account=${account.accountId} outcome=blocked reason=sender_allowlist chat=${diagnosticFingerprint(chatId)} sender=${diagnosticFingerprint(senderId)}`,
    );
    return { allowed: false };
  }
