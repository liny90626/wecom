...
import { diagnosticFingerprint, formatDiagnosticError } from "./diagnostics.js";
...
export async function uploadAndSendMedia(
  ...
  const mediaId = diagnosticFingerprint(mediaUrl);
  const startedAt = Date.now();

  try {
    // 1. 加载媒体文件
    log?.(`[wecom][media] stage=outbound_load_start media=${mediaId} transport=active_send`);
    const media = await resolveMediaFile(mediaUrl, mediaLocalRoots);
    log?.(
      `[wecom][media] stage=outbound_loaded media=${mediaId} transport=active_send bytes=${media.buffer.length} contentType=${media.contentType} filename=${diagnosticFingerprint(media.fileName)}`,
    );
    ...
    if (sizeCheck.shouldReject) {
      errorLog?.(
        `[wecom][media] stage=outbound_rejected media=${mediaId} transport=active_send type=${sizeCheck.finalType} reason=size_limit bytes=${media.buffer.length}`,
      );
      return {
        ok: false,
        rejected: true,
    ...
    log?.(
      `[wecom][media] stage=outbound_uploaded media=${mediaId} transport=active_send upload=${diagnosticFingerprint(uploadResult.media_id)} type=${finalType}`,
    );
    ...
    log?.(
      `[wecom][media] stage=outbound_delivered media=${mediaId} transport=active_send chat=${diagnosticFingerprint(chatId)} type=${finalType} durationMs=${Date.now() - startedAt}`,
    );
    ...
    const errMsg = formatDiagnosticError(err);
    errorLog?.(
      `[wecom][media] stage=outbound_failed media=${mediaId} transport=active_send durationMs=${Date.now() - startedAt} ${errMsg}`,
    );
    return {
      ok: false,
      error: errMsg,
...
export async function uploadAndReplyMedia(
  ...
  try {
    // 1. 加载媒体文件
    log?.(`[wecom][media] stage=outbound_load_start media=${mediaId} transport=reply_media`);
    const media = await resolveMediaFile(mediaUrl, mediaLocalRoots);
    log?.(
      `[wecom][media] stage=outbound_loaded media=${mediaId} transport=reply_media bytes=${media.buffer.length} contentType=${media.contentType} filename=${diagnosticFingerprint(media.fileName)}`,
    );
    ...
    if (sizeCheck.shouldReject) {
      errorLog?.(
        `[wecom][media] stage=outbound_rejected media=${mediaId} transport=reply_media type=${sizeCheck.finalType} reason=size_limit bytes=${media.buffer.length}`,
      );
      return {
        ok: false,
        rejected: true,
    ...
    log?.(
      `[wecom][media] stage=outbound_uploaded media=${mediaId} transport=reply_media upload=${diagnosticFingerprint(uploadResult.media_id)} type=${finalType}`,
    );
    ...
    log?.(
      `[wecom][media] stage=outbound_delivered media=${mediaId} transport=reply_media type=${finalType} durationMs=${Date.now() - startedAt}`,
    );
    ...
    const errMsg = formatDiagnosticError(err);
    errorLog?.(
      `[wecom][media] stage=outbound_failed media=${mediaId} transport=reply_media durationMs=${Date.now() - startedAt} ${errMsg}`,
    );
    return {
      ok: false,
      error: errMsg,
