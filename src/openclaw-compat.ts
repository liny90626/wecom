...
interface SdkExports {
  readRemoteMediaBuffer?: (opts: { url: string; maxBytes?: number }) => Promise<WebMediaResult>;
  detectMime?: (opts: DetectMimeOptions) => Promise<string | undefined>;
  getDefaultMediaLocalRoots?: () => readonly string[];
}

const _sdkReady: Promise<SdkExports> = import("openclaw/plugin-sdk/media-runtime")
  .then((mediaRuntime) => {
    return {
      readRemoteMediaBuffer: mediaRuntime.readRemoteMediaBuffer,
      detectMime: mediaRuntime.detectMime,
      getDefaultMediaLocalRoots: () => mediaRuntime.getAgentScopedMediaLocalRoots({}),
    };
  })
  .catch(() => {
...
void import("openclaw/plugin-sdk/setup")
  .then((sdk) => {
...
async function loadOutboundMediaFromUrlFallback(
  ...
  // Remote URLs are handled only by OpenClaw's guarded media runtime.
  if (/^https?:\/\//i.test(mediaUrl)) {
    throw new Error("OpenClaw guarded remote media loader is unavailable");
  }
...
export async function loadOutboundMediaFromUrl(
  ...
  if (/^https?:\/\//i.test(mediaUrl) && sdk.readRemoteMediaBuffer) {
    return sdk.readRemoteMediaBuffer({
      url: mediaUrl,
