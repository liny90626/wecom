...
function normalizeTemplateCardFields(
  ...
  if (fixes.length > 0) {
    log?.(`[template-card-parser] stage=field_normalization fixes=${fixes.length}`);
  }
...
function validateAndFixRequiredFields(
  ...
  if (fixes.length > 0) {
    log?.(`[template-card-parser] stage=required_field_repair fixes=${fixes.length}`);
  }
  if (warnings.length > 0) {
    log?.(`[template-card-parser] stage=required_field_validation warnings=${warnings.length}`);
  }
...
function transformVoteInteraction(
  ...
  log?.(`[template-card-parser] transformVoteInteraction: complete options=${clampedOptions.length}`);
...
function transformMultipleInteraction(
  ...
  log?.(`[template-card-parser] transformMultipleInteraction: complete selectors=${clampedSelectors.length}`);
...
export function extractTemplateCards(text: string, log?: (...args: any[]) => void): TemplateCardExtractionResult {
  ...
  log?.(`[template-card-parser] stage=extract_start textBytes=${utf8Bytes(text)}`);
  ...
  while ((match = CODE_BLOCK_RE.exec(text)) !== null) {
    ...
    log?.(
      `[template-card-parser] stage=block_found block=${blockIndex} blockBytes=${utf8Bytes(fullMatch)} content=${diagnosticFingerprint(jsonContent)}`,
    );
    ...
    try {
      ...
      // JSON 解析失败，保留在原文中
      log?.(
        `[template-card-parser] stage=block_rejected block=${blockIndex} reason=json_parse ${formatDiagnosticError(e)}`,
      );
      continue;
    }
    ...
    if (typeof cardType !== "string" || !VALID_CARD_TYPES.includes(cardType)) {
      ...
      log?.(
        `[template-card-parser] stage=block_rejected block=${blockIndex} reason=invalid_card_type cardType=${diagnosticFingerprint(cardType)}`,
      );
      continue;
    }

    log?.(`[template-card-parser] stage=block_accepted block=${blockIndex} cardType=${cardType}`);
  ...
  log?.(
    `[template-card-parser] stage=extract_complete cards=${cards.length} remainingBytes=${utf8Bytes(remainingText)}`,
  );
