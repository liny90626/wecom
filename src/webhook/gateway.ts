...
import { diagnosticFingerprint, formatDiagnosticError, utf8Bytes, wecomFlowId } from "../diagnostics.js";
...
/** True only while this account owns registered Bot webhook routes. */
export function isWebhookGatewayRunning(accountId: string): boolean {
  return accountUnregisters.has(accountId);
}
...
export function startWebhookGateway(ctx: WebhookGatewayContext): void {
  ...
  log.info(
    `[wecom][lifecycle] account=${ctx.accountId} stage=webhook_start token=${diagnosticFingerprint(account.token)} receiveId=${diagnosticFingerprint(account.receiveId)} matrixMode=${hasMultiAccounts(ctx.config)}`,
  );
  ...
  log.info(
    `[wecom][lifecycle] account=${ctx.accountId} stage=webhook_ready routeCount=${paths.length} routes=[${paths.join(", ")}]`,
  );
...
export function stopWebhookGateway(ctx: WebhookGatewayContext): void {
  ...
  log.info(`[wecom][lifecycle] account=${ctx.accountId} stage=webhook_stop`);
...
async function flushPending(pending: PendingInbound): Promise<void> {
  ...
  const trace = wecomFlowId({
    accountId: target.account.accountId,
    messageId: msg.msgid || streamId,
  });
  ...
  try {
    ...
    target.runtime.log?.(
      `[wecom][webhook] trace=${trace} stage=batch_runtime_unavailable account=${target.account.accountId} stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`,
    );
    streamStore.markFinished(streamId);
    target.runtime.log?.(
      `[wecom][webhook] trace=${trace} stage=batch_finished account=${target.account.accountId} stream=${diagnosticFingerprint(streamId)} reason=runtime_unavailable`,
    );
  ...
  if (core) {
    ...
    target.runtime.log?.(
      `[wecom][webhook] trace=${trace} stage=batch_started account=${target.account.accountId} stream=${diagnosticFingerprint(streamId)} batch=${diagnosticFingerprint(batchKey)} conversation=${diagnosticFingerprint(conversationKey)} mergedCount=${contents.length} contentBytes=${utf8Bytes(mergedContents)}`,
    );
    ...
    startAgentForStream({
      ...
      streamStore.updateStream(streamId, (state) => {
        state.error = formatDiagnosticError(err);
        state.content = state.content || `Error: ${state.error}`;
        state.finished = true;
      });
      target.runtime.error?.(
        `[wecom][webhook] trace=${trace} stage=batch_failed account=${target.account.accountId} stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`,
      );
      streamStore.onStreamFinished(streamId);
    });
  }
