...
function readBody(
  ...
  return new Promise((resolve) => {
    ...
    req.on("error", (err) => {
      resolve({ ok: false, error: formatDiagnosticError(err) });
    });
  });
}
...
export async function handleWecomWebhookRequest(
  ...
  if (method === "GET") {
    ...
    target.runtime.log?.(
      `[wecom][webhook] request=${reqId} stage=url_verification account=${target.account.accountId} outcome=matched`,
    );
    ...
    try {
      ...
      target.runtime.error?.(
        `[wecom][webhook] request=${reqId} stage=url_verification account=${target.account.accountId} outcome=decrypt_failed ${formatDiagnosticError(err)}`,
      );
  ...
  if (method === "POST") {
    ...
    target.runtime.log?.(
      `[wecom][webhook] request=${reqId} stage=signature_verified account=${target.account.accountId}`,
    );
    ...
    try {
      ...
      target.runtime.error?.(
        `[wecom][webhook] request=${reqId} stage=decrypt_failed account=${target.account.accountId} ${formatDiagnosticError(err)}`,
      );
    ...
    if (expectedBotIds.size > 0) {
      ...
      if (!inboundAibotId || !expectedBotIds.has(inboundAibotId)) {
        target.runtime.error?.(
          `[wecom][webhook] request=${reqId} stage=identity_check account=${target.account.accountId} outcome=mismatch expected=${Array.from(expectedBotIds).map(diagnosticFingerprint).join(",")} actual=${diagnosticFingerprint(inboundAibotId)}`,
        );
      }
    }
    ...
    target.runtime.log?.(
      `[wecom][flow] trace=${traceId} stage=inbound_received account=${target.account.accountId} transport=webhook type=${message.msgtype} message=${diagnosticFingerprint(message.msgid)} sender=${diagnosticFingerprint(resolveWecomSenderUserId(message))} chat=${diagnosticFingerprint(message.chatid)}`,
    );
    ...
    try {
      ...
      target.runtime.log?.(
        `[wecom][flow] trace=${traceId} stage=webhook_reply_complete account=${target.account.accountId} durationMs=${Date.now() - dispatchStartedAt} hasResponse=${Boolean(responseData)}`,
      );
    } catch (err) {
      target.runtime.error?.(
        `[wecom][flow] trace=${traceId} stage=message_failed account=${target.account.accountId} transport=webhook durationMs=${Date.now() - dispatchStartedAt} ${formatDiagnosticError(err)}`,
      );
...
async function dispatchMessage(
  ...
  if (["text", "image", "file", "voice", "video", "mixed"].includes(msgtype)) {
    ...
    if (!filterResult.shouldProcess) {
      target.runtime.log?.(
        `[wecom][webhook] stage=inbound_dropped account=${target.account.accountId} type=${msgtype} reason=${filterResult.reason} sender=${diagnosticFingerprint(resolveWecomSenderUserId(message))} chatType=${String(message.chattype ?? "unknown")}`,
      );
      return null;
    }
