...
import { formatDiagnosticError } from "../diagnostics.js";
    ...
    if (url && aesKey) {
      try {
        ...
        target.runtime.error?.(
          `[wecom][webhook] stage=media_decrypt_failed type=image maxBytes=${maxBytes} ${formatDiagnosticError(err)} hint="increase channels.wecom.media.maxBytes if the payload exceeds the limit"`,
        );
  ...
  if (msgtype === "file") {
    ...
    if (url && aesKey) {
      try {
        ...
        target.runtime.error?.(
          `[wecom][webhook] stage=media_decrypt_failed type=file maxBytes=${maxBytes} ${formatDiagnosticError(err)} hint="increase channels.wecom.media.maxBytes if the payload exceeds the limit"`,
        );
        const errorMessage = formatDecryptError(err);
  ...
  if (msgtype === "video") {
    ...
    if (url && aesKey) {
      try {
        ...
        target.runtime.error?.(
          `[wecom][webhook] stage=media_decrypt_failed type=video maxBytes=${maxBytes} ${formatDiagnosticError(err)} hint="increase channels.wecom.media.maxBytes if the payload exceeds the limit"`,
        );
        const errorMessage = formatDecryptError(err);
        ...
        if (t === "text") {
          ...
          if (!itemAesKey) {
            ...
            try {
              ...
              target.runtime.error?.(
                `[wecom][webhook] stage=media_decrypt_failed type=mixed-${t} maxBytes=${maxBytes} ${formatDiagnosticError(err)} hint="increase channels.wecom.media.maxBytes if the payload exceeds the limit"`,
              );
              const errorMessage = formatDecryptError(err);
