...
function webhookTrace(target: WecomWebhookTarget, messageId?: string, streamId?: string): string {
  return wecomFlowId({
...
export async function handleInboundMessage(
  ...
  const trace = webhookTrace(target, msgid);
  ...
  if (msgid) {
    ...
    if (existingStreamId) {
      ...
      if (existingStream) {
        target.runtime.log?.(
          `[wecom][webhook] trace=${trace} stage=deduplicated account=${target.account.accountId} message=${diagnosticFingerprint(msgid)} stream=${diagnosticFingerprint(existingStreamId)}`,
        );
  ...
  if (!msgContent && !hasMedia(message)) {
    target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=inbound_empty account=${target.account.accountId} type=${message.msgtype} message=${diagnosticFingerprint(msgid)}`);
    return null;
  }
  ...
  target.runtime.log?.(
    `[wecom][webhook] trace=${trace} stage=queued account=${target.account.accountId} status=${status} stream=${diagnosticFingerprint(streamId)} conversation=${diagnosticFingerprint(conversationKey)} contentBytes=${utf8Bytes(msgContent ?? "")}`,
  );
  ...
  if (status === "queued_new") {
    // 进入排队批次，返回排队提示
    target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=queued_next account=${target.account.accountId} stream=${diagnosticFingerprint(streamId)} message=${diagnosticFingerprint(message.msgid)}`);
  ...
  target.runtime.log?.(
    `[wecom][webhook] trace=${trace} stage=queued_merged account=${target.account.accountId} ackStream=${diagnosticFingerprint(ackStreamId)} stream=${diagnosticFingerprint(streamId)} message=${diagnosticFingerprint(message.msgid)}`,
  );
...
export async function handleStreamRefresh(
  ...
  const trace = webhookTrace(target, message.msgid, streamId);
  if (!streamId) {
    target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=stream_refresh_rejected account=${target.account.accountId} reason=missing_stream`);
    return null;
  }

  const stream = state.streamStore.getStream(streamId);
  if (!stream) {
    target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=stream_refresh_missing account=${target.account.accountId} stream=${diagnosticFingerprint(streamId)}`);
  ...
  target.runtime.log?.(
    `[wecom][webhook] trace=${trace} stage=stream_refresh account=${target.account.accountId} stream=${diagnosticFingerprint(streamId)} started=${stream.started} finished=${stream.finished} contentBytes=${utf8Bytes(stream.content)}`,
  );
...
export async function handleEnterChat(
  ...
  target.runtime.log?.(
    `[wecom][webhook] trace=${webhookTrace(target, message.msgid)} stage=enter_chat account=${target.account.accountId} sender=${diagnosticFingerprint(userId)} hasWelcome=${Boolean(welcomeText)}`,
  );
...
export async function handleTemplateCardEvent(
  ...
  if (msgid && streamStore.getStreamByMsgId(msgid)) {
    target.runtime.log?.(
      `[wecom][webhook] trace=${webhookTrace(target, msgid)} stage=template_card_deduplicated account=${target.account.accountId} message=${diagnosticFingerprint(msgid)}`,
    );
    return {};
  }
  ...
  target.runtime.log?.(
    `[wecom][webhook] trace=${webhookTrace(target, msgid)} stage=template_card_received account=${target.account.accountId} event=${diagnosticFingerprint(cardEvent?.event_key)} message=${diagnosticFingerprint(msgid)} selections=${selectedItemList?.length ?? 0}`,
  );
  ...
  startAgentForStream({
    ...
    target.runtime.error?.(`[wecom][webhook] trace=${webhookTrace(target, msgid, streamId)} stage=template_card_agent_failed account=${target.account.accountId} stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`);
  });
...
export async function startAgentForStream(params: {
  ...
  const trace = webhookTrace(target, msg.msgid, streamId);
  ...
  if (!stream) {
    target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=dispatch_skipped account=${target.account.accountId} reason=missing_stream stream=${diagnosticFingerprint(streamId)}`);
    return;
  }
  ...
  const defaultMediaLocalRoots = getAgentScopedMediaLocalRoots(config);
  ...
  if (directLocalPaths.length && looksLikeSendLocalFileIntent(rawBody)) {
    ...
    if (imagePaths.length > 0 && otherPaths.length === 0) {
      ...
      for (const p of imagePaths) {
        try {
          const media = await resolveMediaFile(p, defaultMediaLocalRoots);
          if (!media.contentType.startsWith("image/")) {
            throw new Error("local path does not contain a supported image");
          }
          const base64 = media.buffer.toString("base64");
          const md5 = computeMd5(media.buffer);
          loaded.push({ base64, md5, path: p });
        } catch (err) {
          target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=local_image_read_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} path=${diagnosticPath(p)} ${formatDiagnosticError(err)}`);
        }
      }
      ...
      if (loaded.length > 0) {
        ...
        if (responseUrl) {
          try {
            ...
            target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=local_image_push_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} fallback=stream_refresh ${formatDiagnosticError(err)}`);
          }
        } else {
      ...
      try {
        ...
        target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=fallback_prompt_failed account=${account.accountId} kind=local_image stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`);
      }
        ...
        for (const p of imagePaths) {
          ...
          try {
            await agentDmMedia({
              ...
              mediaLocalRoots: defaultMediaLocalRoots,
            });
            streamStore.updateStream(streamId, (s) => {
              s.agentMediaKeys = Array.from(new Set([...(s.agentMediaKeys ?? []), p]));
            });
          target.runtime.log?.(
              `[wecom][webhook] trace=${trace} stage=local_image_dm_complete account=${account.accountId} stream=${diagnosticFingerprint(streamId)} sender=${diagnosticFingerprint(userid)} path=${diagnosticPath(p)} contentType=${guessedType ?? "unknown"}`,
            );
          } catch (err) {
            target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=local_image_dm_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} path=${diagnosticPath(p)} ${formatDiagnosticError(err)}`);
          }
        }
      }
      streamStore.onStreamFinished(streamId);
    ...
    if (otherPaths.length > 0) {
      ...
      try {
        ...
        target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=fallback_prompt_failed account=${account.accountId} kind=local_file stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`);
      }
      ...
      for (const p of otherPaths) {
        ...
        try {
          ...
          target.runtime.log?.(
            `[wecom][webhook] trace=${trace} stage=local_file_dm_complete account=${account.accountId} stream=${diagnosticFingerprint(streamId)} sender=${diagnosticFingerprint(userid)} path=${diagnosticPath(p)} contentType=${guessedType ?? "unknown"}`,
          );
        } catch (err) {
          target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=local_file_dm_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} path=${diagnosticPath(p)} ${formatDiagnosticError(err)}`);
        }
      }
      streamStore.onStreamFinished(streamId);
      return;
  ...
  if (media) {
    try {
      ...
      target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=inbound_media_saved account=${account.accountId} stream=${diagnosticFingerprint(streamId)} path=${diagnosticPath(mediaPath)} type=${mediaType} bytes=${media.buffer.length}`);
    } catch (err) {
      target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=inbound_media_save_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`);
    }
  }
  ...
  if (mediaPath && mediaType?.startsWith("video/")) {
    try {
      ...
      if (videoFirstFramePath) {
        target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=video_frame_ready account=${account.accountId} stream=${diagnosticFingerprint(streamId)} path=${diagnosticPath(videoFirstFramePath)}`);
      }
    } catch (err) {
      target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=video_frame_unavailable account=${account.accountId} stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`);
    }
  }
  ...
  const agentMediaLocalRoots = getAgentScopedMediaLocalRoots(config, route.agentId);
  // ===== 动态 Agent 路由处理结束 =====
  // ──────────────────────────────────────────────────────────────────
  ...
  target.runtime.log?.(
    `[wecom][webhook] trace=${trace} stage=authorization account=${account.accountId} dmPolicy=${authz.dmPolicy} shouldCompute=${authz.shouldComputeAuth} sender=${diagnosticFingerprint(userid.toLowerCase())} senderAllowed=${authz.senderAllowed} authorizerConfigured=${authz.authorizerConfigured} commandAuthorized=${String(commandAuthorized)}`,
  );
  ...
  if (authz.shouldComputeAuth && authz.commandAuthorized !== true) {
    ...
    try {
      await sendBotFallbackPromptNow({ streamId, text: prompt });
      target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=authorization_rejected account=${account.accountId} stream=${diagnosticFingerprint(streamId)} userNotified=true`);
    } catch (err) {
      target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=authorization_notice_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`);
    }
    streamStore.onStreamFinished(streamId);
    return;
  }
  ...
  await core.channel.session.recordInboundSession({
    ...
    onRecordError: (err: unknown) => {
      target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=session_record_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`);
    },
  });
  ...
  await core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
    ...
    dispatcherOptions: {
      deliver: async (payload: { text?: string; mediaUrl?: string; mediaUrls?: string[] }, info: { kind: string }) => {
          ...
          try {
            ...
            if (parsed.template_card) {
              ...
              if (responseUrl && isSingleChat) {
                ...
                target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=template_card_sent account=${account.accountId} stream=${diagnosticFingerprint(streamId)} task=${diagnosticFingerprint(parsed.template_card.task_id)}`);
                streamStore.updateStream(streamId, (s) => {
                  s.finished = true;
                  s.content = "[已发送交互卡片]";
                });
        ...
        if (!payload.mediaUrl && !(payload.mediaUrls?.length ?? 0) && text.includes("/")) {
          ...
          if (candidates.length > 0) {
            ...
            for (const p of candidates) {
              try {
                const media = await resolveMediaFile(p, agentMediaLocalRoots);
                if (!media.contentType.startsWith("image/")) {
                  continue;
                }
                const base64 = media.buffer.toString("base64");
                ...
                target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=output_image_read_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} path=${diagnosticPath(p)} ${formatDiagnosticError(err)}`);
              }
            }
          }
        }
        ...
        if (nearTimeout) {
          ...
          try {
            ...
            target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=timeout_notice_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`);
          }
          return;
        }
        ...
        for (const mPath of mediaUrls) {
          ...
          try {
            const loaded = await resolveMediaFile(mPath, agentMediaLocalRoots);
            ...
            if (contentType?.startsWith("image/")) {
              ...
              current.images.push({ base64, md5 });
              target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=output_image_ready account=${account.accountId} stream=${diagnosticFingerprint(streamId)} source=${diagnosticPath(mPath)} contentType=${contentType} bytes=${buf.length}`);
            } else {
              // Webhook 模式：统一切换到 Agent 私信兜底，并在 Bot 会话里提示用户。
              const agentOk = isAgentConfigured(target);
              const alreadySent = current.agentMediaKeys.includes(mPath);
              target.runtime.log?.(
                `[wecom][webhook] trace=${trace} stage=output_file_fallback account=${account.accountId} stream=${diagnosticFingerprint(streamId)} source=${diagnosticPath(mPath)} chatType=${current.chatType} contentType=${contentType ?? "unknown"} agentConfigured=${agentOk} alreadySent=${alreadySent} hasResponseUrl=${Boolean(getActiveReplyUrl(streamId))}`,
              );
              ...
              if (agentOk && !alreadySent && current.userId) {
                try {
                  await agentDmMedia({
                    ...
                    contentType,
                    filename,
                    mediaLocalRoots: agentMediaLocalRoots,
                    resolvedMedia: loaded,
                  });
                  target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=output_file_dm_complete account=${account.accountId} stream=${diagnosticFingerprint(streamId)} sender=${diagnosticFingerprint(current.userId)}`);
                  streamStore.updateStream(streamId, (s) => {
                    s.agentMediaKeys = Array.from(new Set([...(s.agentMediaKeys ?? []), mPath]));
                  });
                } catch (err) {
                  target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=output_file_dm_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`);
                }
              }
              ...
              if (!current.fallbackMode) {
                ...
                try {
                  ...
                  target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=media_notice_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`);
                }
              }
              return;
            }
          } catch (err) {
            target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=output_media_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} source=${diagnosticPath(mPath)} ${formatDiagnosticError(err)}`);
            ...
            if (agentOk && current.userId && !current.agentMediaKeys.includes(mPath)) {
              try {
                ...
                target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=output_media_recovery_dm_complete account=${account.accountId} stream=${diagnosticFingerprint(streamId)} sender=${diagnosticFingerprint(current.userId)}`);
              } catch (sendErr) {
            ...
            if (!current.fallbackMode) {
              ...
              try {
                ...
                target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=output_media_recovery_notice_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(pushErr)}`);
              }
            }
            return;
          }
      ...
      onError: (err: unknown) => {
        target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=agent_reply_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`);
      },
    },
  });
  ...
  if (finishedState?.fallbackMode === "timeout" && !finishedState.finalDeliveredAt) {
    if (!isAgentConfigured(target)) {
      ...
      if (dmText) {
        try {
          target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=timeout_dm_start account=${account.accountId} stream=${diagnosticFingerprint(streamId)} sender=${diagnosticFingerprint(finishedState.userId)} contentBytes=${utf8Bytes(dmText)}`);
          await agentDmText({ target, userId: finishedState.userId, text: dmText });
          target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=timeout_dm_complete account=${account.accountId} stream=${diagnosticFingerprint(streamId)} sender=${diagnosticFingerprint(finishedState.userId)}`);
        } catch (err) {
          target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=timeout_dm_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} ${formatDiagnosticError(err)}`);
        }
      }
      streamStore.updateStream(streamId, (s) => { s.finalDeliveredAt = Date.now(); });
    }
  ...
  if (stateAfterFinish && responseUrl) {
    try {
      ...
      target.runtime.log?.(
        `[wecom][webhook] trace=${trace} stage=final_push_complete account=${account.accountId} stream=${diagnosticFingerprint(streamId)} chatType=${chatType} images=${stateAfterFinish.images?.length ?? 0} durationMs=${Date.now() - startedAt}`,
      );
    } catch (err) {
      target.runtime.error?.(`[wecom][webhook] trace=${trace} stage=final_push_failed account=${account.accountId} stream=${diagnosticFingerprint(streamId)} durationMs=${Date.now() - startedAt} ${formatDiagnosticError(err)}`);
    }
  }
  ...
  // 16. 更新回执流 + 推进队列
  // ──────────────────────────────────────────────────────────────────
  target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=batch_finished account=${account.accountId} stream=${diagnosticFingerprint(streamId)} durationMs=${Date.now() - startedAt}`);
  ...
  if (ackStreamIds.length > 0) {
    ...
    target.runtime.log?.(`[wecom][webhook] trace=${trace} stage=ack_streams_finished account=${account.accountId} stream=${diagnosticFingerprint(streamId)} count=${ackStreamIds.length}`);
  }
...
async function agentDmMedia(params: {
  ...
  const { target, userId, mediaUrlOrPath } = params;
  if (!target.account.agent?.configured) {
    throw new Error("Agent credentials not configured");
  }
  const agent = target.account.agent;
  const media = params.resolvedMedia
    ?? await resolveMediaFile(mediaUrlOrPath, params.mediaLocalRoots);
  const contentType = params.contentType || media.contentType;
  const detectedType = detectWeComMediaType(contentType);
  const sizeCheck = applyFileSizeLimits(media.buffer.length, detectedType, contentType);
  if (sizeCheck.shouldReject) {
    throw new Error(sizeCheck.rejectReason ?? "企业微信媒体文件超过允许大小");
  }
  const mediaType = sizeCheck.finalType;
  const filename = params.filename || media.fileName;
